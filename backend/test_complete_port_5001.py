...
def test_complete_port_5001():
    print("Testing Complete System on Port 5001...")
    print("=" * 50)
    ...
    try:
        ...
        response = requests.get('http://127.0.0.1:5001/api/admin/users', headers=headers, timeout=5)
        ...
        for endpoint in family_endpoints:
            try:
                response = requests.get(f'http://127.0.0.1:5001{endpoint}', headers=headers, timeout=5)
        ...
        print("\n" + "=" * 50)
        print("[SUCCESS] All systems working on port 5001!")
        print("Frontend should now connect to http://127.0.0.1:5001")
