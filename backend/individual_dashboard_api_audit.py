...
def audit_individual_dashboard_apis():
    """
    Comprehensive audit of Individual Dashboard API endpoints
    Checks frontend components against backend endpoints
    """
    print("=" * 80)
    print("INDIVIDUAL DASHBOARD API ENDPOINT AUDIT REPORT")
    print("=" * 80)
...
def extract_frontend_api_calls(frontend_path):
    ...
    # Search through user components only
    user_path = os.path.join(frontend_path, 'components', 'user')
...
def generate_report(backend_endpoints, frontend_calls, issues):
    ...
    report_file = os.path.join(os.path.dirname(__file__), 'INDIVIDUAL_DASHBOARD_API_AUDIT_REPORT.md')
    
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("# Individual Dashboard API Endpoint Audit Report\n\n")
...
if __name__ == "__main__":
    audit_individual_dashboard_apis()
