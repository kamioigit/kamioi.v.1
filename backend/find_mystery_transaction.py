#!/usr/bin/env python3
"""
Find the mystery transaction that's showing up in the API
"""
...
def find_mystery():
    """Find the mystery transaction"""
    print("[FIND MYSTERY] Finding mystery transaction...")
    ...
    try:
        if db_manager._use_postgresql:
            ...
            if total > 0:
                ...
                print("\n   [WARNING] Database shows 0 transactions, but API returns 1!")
                print("   This suggests the API might be using a cached connection or different database.")
            
            # Check round_up_allocations
            print("\n3. Checking round_up_allocations...")
            try:
                result = conn.execute(text('SELECT COUNT(*) FROM round_up_allocations'))
                total_roundups = result.scalar() or 0
                print(f"   Total round_up_allocations in database: {total_roundups}")
                
                if total_roundups > 0:
                    result = conn.execute(text('SELECT id, transaction_id FROM round_up_allocations ORDER BY id'))
                    for row in result:
                        print(f"   - ID: {row[0]}, Transaction ID: {row[1]}")
                else:
                    print("   [WARNING] Database shows 0 round_up_allocations, but API returns 12!")
