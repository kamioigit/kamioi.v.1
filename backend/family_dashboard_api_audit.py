...
def audit_family_dashboard_apis():
    """
    Comprehensive audit of Family Dashboard API endpoints
    Checks frontend components against backend endpoints
    """
    print("=" * 80)
    print("FAMILY DASHBOARD API ENDPOINT AUDIT REPORT")
    print("=" * 80)
...
def extract_frontend_api_calls(frontend_path):
    ...
    # Search through family components only
    family_path = os.path.join(frontend_path, 'components', 'family')
...
def generate_report(backend_endpoints, frontend_calls, issues):
    ...
    report_file = os.path.join(os.path.dirname(__file__), 'FAMILY_DASHBOARD_API_AUDIT_REPORT.md')
    
    with open(report_file, 'w', encoding='utf-8') as f:
        f.write("# Family Dashboard API Endpoint Audit Report\n\n")
